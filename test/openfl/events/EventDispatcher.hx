...
class EventDispatcher implements IEventDispatcher
{
	...
	**/
	public function addEventListener<T>(type:EventType<T>, listener:T->Void, useCapture:Bool = false, priority:Int = 0, useWeakReference:Bool = false):Void
	{
		...
		else
		{
			...
			__addListenerByPriority(list, new Listener(listener, useCapture, priority),type);
		}
	}
	...
	**/
	 ...
	 * @return Bool
	 */
	public function dispatchEventWithThread(event:Event):Bool{
	...
	**/
	...
	@:noCompletion private function __addListenerByPriority<T>(list:Array<Listener>, listener:Listener,type:EventType<T>):Void
	{
